...
# Main-branch integration, 25 September 2026

The `idtech5-ui` integration retains main's single-player cvar ownership fixes
and the branch's HDR, bot, weapon and UI changes. Engine-facing renderer and
system headers match the merged engine, including retained font services and
PBR material layout. Game API 49 rejects older modules with incompatible layouts.
Build and replace both SP and MP modules with the matching engine and renderers.
The complete UI replacement remains in development.

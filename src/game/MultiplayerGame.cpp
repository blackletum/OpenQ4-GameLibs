...
// RAVEN END
		...
		if ( nameSourcePlayer && nameSourcePlayer->GetUserInfo() ) {
			cvarSystem->SetCVarString( "gui_ui_name", nameSourcePlayer->GetUserInfo()->GetString( "ui_name" ) );
			cvarSystem->SetCVarString( "gui_ui_clan", nameSourcePlayer->GetUserInfo()->GetString( "ui_clan" ) );
		} else {
...
void idMultiplayerGame::ReadFromSnapshot( const idBitMsgDelta &msg ) {
	...
	bool		decodedBuyingAllowed = false;
	int			decodedPowerupCount = 0;
	float		decodedMarinePulse = 0.0f;
	float		decodedStroggPulse = 0.0f;
...
// RITUAL END
	...
	for ( i = 0; i < MAX_CLIENTS; i++ ) {
		if ( decodedPlayerState[i].ingame ) {
			...
			if ( proto69 ) {
				decodedPlayerState[ i ].deadZoneScore = 0;
			} else {
